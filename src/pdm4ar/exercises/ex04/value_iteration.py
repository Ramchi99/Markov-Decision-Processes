...
from pdm4ar.exercises.ex04.structures import Policy, ValueFunc, Action, Cell, State
...
class ValueIteration(GridMdpSolver):
    ...
    def solve(grid_mdp: GridMdp) -> tuple[ValueFunc, Policy]:
        ...
        # Precompute transitions for each (state, action) pair we will consider.
        transitions = {}
        P = {}
        R = {}
        ...
        ordered_states = [
            s for s in np.ndindex(grid_mdp.grid.shape) if grid_mdp.grid[s] not in (Cell.CLIFF, Cell.WONDERLAND)
        ]
        ...
        while True:
            ...
            for state in ordered_states:
                ...
                # get the precomputed admissible actions for this state
                valid_actions_for_state = valid_actions[state]

                Q_sa = [Q(state, a) for a in valid_actions_for_state]

                V_s_new = max(Q_sa)
                delta = max(delta, abs(value_func[state] - V_s_new))
                value_func[state] = V_s_new
                policy[state] = valid_actions_for_state[int(np.argmax(Q_sa))]
        ...
        for state in np.ndindex(grid_mdp.grid.shape):
            ...
            for action in actions:
                for next_state in neighbors[state]:
                    # for next_state in np.ndindex(grid_mdp.grid.shape):
                    prob = grid_mdp.get_transition_prob(state, action, next_state)
                    if prob > 0:
                        # test --> neighbors list correctly catches every non zero prob.
                        if next_state not in neighbors[state]:
                            print(prob, grid_mdp.stage_reward(state, action, next_state))
                        P[(state, action, next_state)] = prob
        ...
        while True:
            ...
            for state in np.ndindex(grid_mdp.grid.shape):
                ...
                # --- determine valid actions for this state ---
                valid_actions = [Action.ABANDON]
                for action in actions_no_abandon:
                    # 1️⃣ Only allow Stay if at goal
                    if action == Action.STAY and cell != Cell.GOAL:
                        continue

                    # 2️⃣ Compute next cell for this action
                    dr, dc = change[action]
                    nr, nc = r + dr, c + dc
                    ...
                    # Skip if leads into a cliff
                    if grid_mdp.grid[nr, nc] == Cell.CLIFF:
                        continue

                    valid_actions.append(action)

                Q_sa = [Q(state, a) for a in valid_actions]

                V_s_new = max(Q_sa)
                delta = max(delta, abs(value_func[state] - V_s_new))
                value_func[state] = V_s_new
                policy[state] = valid_actions[int(np.argmax(Q_sa))]

                # policy[state] = int(np.argmax(Q_sa))
