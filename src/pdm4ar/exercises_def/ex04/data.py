...
def get_test_grids(evaluation_tests: list[tuple[tuple[int, int], int, int, int]] = []) -> list[GridMdp]:
    ...
    #test_maps.append(generate_map(MAP_SHAPE_3, swamp_ratio, n_wonderland=5, n_cliff=10, n_seed=140))
...
def get_transition_prob_test_cases(grid_mdps: list[GridMdp]) -> list[TestTransitionProbEx4]:
    ...
    for grid_idx, grid_mdp in enumerate(grid_mdps):
        ...
        if grid_idx == 0:  # Simple 5x5 test grid
            ...
            """
            # --- NEW BRUTE-FORCE TEST CASE GENERATION ---
            test_scenarios = []
            for i in range(rows):
                for j in range(cols):
                    for next_i in range(rows):
                        for next_j in range(cols):
                            for action in Action:
                                test_scenarios.append(((i, j), action, (next_i, next_j)))
            """
...
def get_expected_results_transition(test_cases: list[TestTransitionProbEx4]) -> list[float]:
    ...
    res: list[float] = []
    filtered_test_cases: list[TestTransitionProbEx4] = []

    for test in test_cases:
        state = test.state
        action = test.action
        next_state = test.next_state
        val = transition_probs.get((state, action, next_state), -1)
        if val != -1:
            res.append(val)
    ...
    test_cases.clear()
    test_cases.extend(filtered_test_cases)
